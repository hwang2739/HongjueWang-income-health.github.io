...
```{r, message = FALSE, warning = FALSE, fig.asp = 1, fig.width=6, fig.height=10}
...
ggplotly(p1, tooltip = "text", height = 350, width=600) %>% 
...
ggplotly(p2, tooltip = "text", height = 350, width=600) %>% 
  config(displayModeBar = FALSE) %>%
  layout(hoverlabel = list(bgcolor = "white", font = list(family = "Georgia")))
```
...
leaflet(states.merged, height="100%", width="100%") %>% addTiles() %>%
  setView(-96, 37.8, 4) %>%
